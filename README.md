...
- **`/v2`** is a website that runs on its own [Vercel Sandbox](https://vercel.com/docs/sandbox). You select elements, draw on them, and leave notes while you talk, and the agent edits its pages, which are plain HTML, CSS, and JavaScript files at their own URLs.
...
`/v2` and `/v3` also need Vercel Sandbox, so link a Vercel project (`vercel link && vercel env pull`). That's all `/v2` needs: each design's preview is a small static server, [`server.mjs`](src/app/v2/_preview/server.mjs), that starts on port 3000 of a fresh sandbox in a couple of seconds. For `/v3`, also build the desktop snapshot once, and set `MARGIN_V3_SNAPSHOT_ID` to the id it prints:
