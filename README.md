...
Browser tests use a simulated Gateway WebSocket while running the real editor, tool executor, preview capture, and browser microphone pipeline. They cover voice-only interaction, 24 kHz configuration, mute/unmute, cancellation of pending microphone access, malformed tool recovery, interruption, duplicate calls, and literal HTML replacements with undo. A live check on September 16 sent synthesized speech asking for the heading to say “Great” through the local Gateway connection: GPT Realtime 2 used one `edit_html` call with 154 characters of arguments and updated the heading with no selection. This used an API key, not OIDC-only authentication. Model design quality still needs hands-on evaluation.
