  ...
  const render = () => {
    ...
    const positions = notes.map((note, index) => {
      ...
      if (visible) {
        ...
        pin.style.cssText = `position:fixed;left:${Math.max(4, Math.min(innerWidth - 28, rect.right - 12))}px;top:${Math.max(4, rect.top - 12)}px;width:24px;height:24px;border:2px solid white;border-radius:50%;background:#171717;color:white;font:11px Arial,sans-serif;pointer-events:auto;cursor:pointer;box-shadow:0 2px 8px #0004`
        pin.onclick = () =>
  ...
  window.addEventListener('message', (event) => {
    ...
    if (!root) {
      ...
      hoverBox.style.cssText =
        'position:fixed;border:1px solid #0070f3;border-radius:2px;pointer-events:none;box-sizing:border-box'
      selectionBox.style.cssText =
        'position:fixed;border:2px solid #0070f3;background:#0070f308;border-radius:2px;pointer-events:none;box-sizing:border-box'
