...
import { STUDIO_INSTRUCTIONS } from './tools'
import type {
  AnnotationPosition,
  Design,
  ElementTarget,
...
export function Studio({
  ...
  const ensureSite = useCallback(async () => {
    ...
    initializing.current ??= api
      .createDesign()
      .then(async (site) => {
        showSite(site)
        setDesigns(await api.designs())
        return site
      })
      .finally(() => {
    ...
    async (annotation: SiteAnnotation) => {
      ...
      try {
        await mutate(async () => {
          ...
          const annotations = await api.saveAnnotation(site.id, annotation)
    ...
    async (name: string, args: unknown, signal: AbortSignal) => {
      ...
      if (name === 'edit_html' || name === 'write_html')
        return mutate(async () => {
          ...
          setSaving(true)
          try {
            const { site, matches, missed } = await api.editSite(
  ...
  const openDesign = async (id: string | null) => {
    ...
    try {
      ...
      await mutations.current.catch(() => {})
      const site = await (id ? api.site(id) : api.createDesign())
      if (version !== navigation.current) return
      showSite(site)
      setDraftDrawings([])
      setFailedDrawings([])
  ...
  const addNote = async (comment: string) => {
    ...
    try {
      ...
      const annotations = await mutate(() =>
        api.saveAnnotation(site.id, annotation),
      )
      setSite((previous) =>
        previous ? { ...previous, annotations } : previous,
      )
  ...
  const deleteNote = async (id: string) => {
    try {
      const annotations = await mutate(() =>
        api.deleteAnnotation(snapshot.current.site?.id ?? null, id),
      )
      setSite((previous) =>
        previous ? { ...previous, annotations } : previous,
      )
